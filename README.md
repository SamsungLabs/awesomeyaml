...
Briefly speaking, yamlfig is capable of:
...
You can install yamlfig by cloning this repo and installing it with pip:
```bash
git clone https://github.sec.samsung.net/SAIC-Cambridge/yamlfig.git
python -m pip install ./yamlfig
```

Optionally, you can add `-e` option to allow for in-place (also called editable or developer) installation:

```bash
python -m pip install -e ./yamlfig
```

or, if you don't need the repo to be laying around simply install directly from github:

```bash
python -m pip install https://github.sec.samsung.net/SAIC-Cambridge/yamlfig/tarball/master#egg=yamlfig
```
...
Since yamlfig is a pure python library with minimal dependencies and does not directly depend on any OS or HW specific components, it is rather unlikely that a version tested on one machine would break on another.
...
Use `yamlfig.Config` for default, out-of-the-box behaviour.
...
with open('file2.yaml', 'r') as f:
    ...
    cfg = yamlfig.Config.build('file1.yaml', f, 'field: !!null')
```

alternatively, you can let yamlfig parse part of your command line arguments and handle filenames and inline overwrites:
...
cfg = yamlfig.Config.build_from_cmdline(*args.yamls, filename_lookup_fn=lookup)
```

Access fields in a dict by either standard indexing operator or by attribute names: `cfg['field']` is the same as `cfg.field`; consequently, access yamlfig API via a proxy object (namespace), e.g.: `cfg.yamlfigns.children()`

## Low-level API
Use `yamlfig.Builder` to merge multiple yaml sources without evaluating dynamic nodes.
Use `yamlfig.EvalContext` to evaluate merged config:

```python
import yamlfig

# merge multiple files
builder = yamlfig.Builder()
builder.add_source('file1.yaml')
builder.add_multiple_sources('file2.yaml', 'field: !!null')
cfg = builder.build()

# evaluate dynamic nodes in cfg
eval_ctx = yamlfig.EvalContext(cfg)
cfg = eval_ctx.evaluate()
# at this point cfg should be roughly the same as cfg from the first example
```

Use `yamlfig.yaml` for core functionality of reading and writing yaml files and to add support for custom tags.

```python
import yamlfig
import yamlfig.yaml as yyaml

#
# Read and write files with support for yamlfig tags
#
...
with open('file1.yaml', 'r') as f:
    ...
    # you can also pass yamlfig.Builder instance
    # for more complete functionality,
    # this is what happens normally when using
    # higher-level functions
    obj = yyaml.parse(f.read())

yyaml.dump(obj, 'file1_copy.yaml')
...
# returned value should be derived from `yamlfig.nodes.ConfigNode
class MyNodeType(yamlfig.nodes.ConfigNode):
    pass
...
# yyaml.yaml is the standard yaml package used by yamlfig
yyaml.yaml.add_constructor('!mynode', my_node_constructor)
```

See the [summary of the extended tags](#Summary-of-extended-tags) for an overview of extra tags implemented by yamlfig.
...
As mentioned before, to help dealing with situations like that, yamlfig is designed to work with configurations which are split into multiple files.
...
Merging of the yaml files is handed by `yamlfig.Builder` and evaluation is done by `yamlfig.EvalContext`, with `yamlfig.Config` being the top-level class encapsulating both steps in a standard way presented in the code snippet above.

The following documentation is split into two parts.
The first talks about working with yaml files and how yamlfig functionality helps with that.
The second concerns yamlfig Python's API and how to achieve the behaviour explained in the first part.
...
The only unusual thing is the non-standard `!required` tag - this tag is provided by yamlfig and can be used to identify parameters which are introduced (and required) by a specific part of the system but should be defined by other parts of the system.
...
Here, `!include` is one of the extended tags introduced by yamlfig.
...
Apart from merging, another important aspect of yamlfig is support for lazily-evaluated nodes.
...
Therefore, even though at its core yamlfig will use the aforementioned mechanism to store and read metadata, it also provides a human-friendly way of defining metadata by extending yaml syntax a little bit.
...
Following the example command lines above, the desired behaviour could be achieved by using `yamlfig.Config.build` static method, for example:
...
cfg = yamlfig.Config.build('configs/models/model1.yaml',
...
cfg = yamlfig.Config.build(*sys.argv[1:])
```

### Handling command-line arguments

Please note that yamlfig does not perform any soffisticated command line parsing, so the example above with `sys.argv` can easily break it.
Specifically, the `yamlfig.Config.build` method expects all its arguments to be either of:
 - a filename
 - a file object
 - yaml string

Because of that, it's advised to use `argparse` to extract arguments which are meaningful for yamlfig and preprocess the rest independently.

However, there is one extra thing yamlfig is capable of with regard to command line processing - it can construct valid yaml from one-liners which can be used to overwrite a specific value. Consider the following example: we'd like to train `model1` with `dataset2` and `optim2` (as in the examples before) but we want to experiment with different number of channels. It is impractical to create a new config file each time we want to change just a single value, so we try to overwrite the default values by appending some yaml to the command line:
...
However, expressions like the one above are not valid yaml and hence cannot be used directly with `yamlfig.Config.build`.
Instead, yamlfig provides another function `yamlfig.Config.build_from_cmdline` which is capable of identifying and transforming those expressions into their full-yaml form (like the one in the first example) suitable to be given to the standard `build` function.
...
Specifically, because it is possible in Python to shadow methods and other class-level attributes with instance-level attributes, it is fairly easy to introduce undesired conficts between attributes introduced by reading the user's config file and either internal or external API of yamlfig's entities.

For example, [ComposedNode](yamlfig/nodes/composed.py) class, which is the base class for both lists and dicts within yamlfig, provides a common interface for iterating and manipulating sub-nodes of a node. One of the methods introduced by this class is called `children` and can be used to simply iterate over sub-nodes.
...
In that case, any code which would access `obj.children` with the intention of calling the yamlfig function would instead access the value read from the config file, most likely resulting in an error.
...
To avoid name conflicts and still be able to use the bunch pattern, we decided to introduce two rules:
 ...
 - all other methods and attributes which are part of the yamlfig API are put under `yamlfigns` namespace - in order to access them, one needs to go through the proxy called `yamlfigns`, e.g. to access the previously mentioned `children` method, the relevant call would look like: `cfg.yamlfigns.children()`
 > **Note:** it is still possible to shadow `yamlfigns` name with a value from a config file as there are no checks to prevent this, therefore the user should be careful about (not) using this name - including it in a config file would result in undefined behaviour.


## Summary of extended tags
This section includes a brief summary of the new tags introduced by yamlfig, each with a short description and a link to more information.
...
| Tag | Description | Doc |
|-|-|-|
| `!append [list]` | Appends to an existing list on merge | [AppendNode](docs/build/html/yamlfig.nodes.append.html) |
| `!bind:name { args }` | Binds arguments `args` to a python entity called `name` | [BindNode](docs/build/html/yamlfig.nodes.bind.html) |
| `!call:name { args }` | Calls a python entity called `name` with arguments `args` | [CallNode](docs/build/html/yamlfig.nodes.call.html) |
| `!eval code` | Evaluates arbitrary python `code` | [EvalNode](docs/build/html/yamlfig.nodes.eval.html) |
| `!fstr str` | Evaluates an f-string expression `str` | [FStrNode](docs/build/html/yamlfig.nodes.fstr.html) |
| `!import name` | Embeds a python entity called `name` | [ImportNode](docs/build/html/yamlfig.nodes.import.html) |
| `!include file` | Includes another config `file` recursively | [IncludeNode](docs/build/html/yamlfig.nodes.include.html) |
| `!path:ref [joins]` | Computes a path, starting with a reference point `ref` and then following a list of folders in `joins` | [PathNode](docs/build/html/yamlfig.nodes.path.html) |
| `!prev node` | Moves the content of `node` to a new place on merge | [PrevNode](docs/build/html/yamlfig.nodes.prev.html) |
| `!required` | Specifies that a value should be provided later | [RequiredNode](docs/build/html/yamlfig.nodes.required.html) |
| `!xref node` | Cross-references another node | [XRefNode](docs/build/html/yamlfig.nodes.xref.html) |
